...
class _MyHomePageState extends State<MyHomePage>
  ...
  Future<void> _fetchUserData() async {
    ...
    try {
      ...
      if (doc.exists) {
        userData =
            doc.data() as Map<String, dynamic>; // Store the entire userData
      ...
      if (doc.exists) {
        ...
        if (dietPlan.containsKey(dayKey)) {
          ...
          if (dayData is String) {
            ...
            print(
              "First 100 chars: ${dayData.substring(0, min(100, dayData.length))}",
            );

            // Updated regex pattern to better match your **MealType:** format
            final pattern =
                r'\*\*' + _currentMealType + r':\*\*(.*?)(?=\*\*\w+:|$)';
            ...
            if (match != null) {
              ...
              _currentMealContent =
                  match.group(1)?.trim() ?? 'No details available';
              print("Raw match content: $_currentMealContent");
            } else {
              print("NO REGEX MATCH FOUND!");
              _currentMealContent =
                  "Details for $_currentMealType not found in your meal plan.";
            }
          }
          // Replace your existing meal extraction logic with this specific version:
            ...
            if (dayData.containsKey('plan')) {
              ...
              if (planData is String) {
                ...
                print(
                  "Plan content sample: ${planData.length > 200 ? '${planData.substring(0, 200)}...' : planData}",
                );
                ...
                final allMeals =
                    RegExp(
                      r'\*\*(\w+\s*\w*)\*\*',
                    ).allMatches(planData).map((m) => m.group(1)).toList();
                ...
                if (match != null && match.groupCount >= 1) {
                  ...
                  print(
                    "Raw content first 50 chars: ${rawContent.length > 50 ? rawContent.substring(0, 50) : rawContent}",
                  );
                  ...
                  if (_currentMealContent.isEmpty) {
                    ...
                    print(
                      "Fallback content length: ${_currentMealContent.length}",
                    );
                  }
                } else {
                  ...
                  if (_currentMealType == "Snack") {
                    ...
                    for (final name in alternateNames) {
                      ...
                      if (altMatch != null && altMatch.groupCount >= 1) {
                        ...
                        print(
                          "Content preview: ${_currentMealContent.substring(0, min(50, _currentMealContent.length))}...",
                        );
                        break;
                      }
                    }
          ...
          _currentMealContent =
              "No meal plan found for today (Day $_currentDayNum).";
        }
      } else {
        print("Diet document does NOT exist");
        _currentMealContent =
            "No meal plan found. Generate one from the Meal Plan page.";
      }
    } catch (e, stackTrace) {
        ...
        if (workoutPlan.containsKey(dayKey)) {
          ...
          if (dayData is Map && dayData.containsKey('plan')) {
            ...
            if (planData is String) {
              ...
              final dayTitlePattern =
                  r'### Day ' + _currentDayNum.toString() + r': ([^\n]+)';
              final dayTitleMatch = RegExp(
                dayTitlePattern,
              ).firstMatch(planData);

              if (dayTitleMatch != null && dayTitleMatch.groupCount >= 1) {
                _currentWorkoutType =
                    dayTitleMatch.group(1)?.trim() ?? _currentWorkoutType;
                print(
                  "Extracted workout type from title: $_currentWorkoutType",
                );
              }
            }
          }
          ...
          _currentWorkoutContent =
              ""; // Empty string instead of the "Tap to see..." message
        }
      } else {
        _currentWorkoutContent =
            "No workout plan found. Generate one from the Workout page.";
      }
    } catch (e) {
                                      ...
                                      builder:
                                          (context) => Container(
                                            height:
                                                ...
                                                0.85, // Increased from 0.6 to 0.85 (85% of screen)
                                            margin: EdgeInsets.only(
                                              top: 50,
                                            ), // Reduced top margin to allow more space for profile
                                            decoration: BoxDecoration(
                                            ...
                                            child: ClipRRect(
                                              ...
                                              child: ProfileScreen(
                                                scrollController:
                                                    ScrollController(), // Provide a basic scroll controller
                                              ),
                                            ),
                                          ),
                                    );
                        ...
                        Column(
                          children: [
                            ...
                            InkWell(
                              ...
                              child: Container(
                                height:
                                    235, // Taller container for more content
                                width: double.infinity,
                                decoration: BoxDecoration(
                                  borderRadius: BorderRadius.circular(16),
                                  color: theme.colorScheme.primary.withOpacity(
